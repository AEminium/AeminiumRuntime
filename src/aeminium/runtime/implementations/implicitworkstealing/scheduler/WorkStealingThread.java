...
import aeminium.runtime.tasktype.TaskType;
...
public final class WorkStealingThread extends AeminiumThread {
	...
	/*
	 * Main loop that executes tasks from queue, steals if empty, or parks until
	 * some task awakes it.
	 */
	@Override
	public final void run() {
		...
		while (!shutdown) {
			...
			if (task != null) {
				// statistics
				rt.scheduler.setStatistics(task, index, "taskQueue");
				// ************
				...
				if (task != null) {
					// statistics
					rt.scheduler.setStatistics(task, index, "stealQueue");
					// **********

					task.invoke(rt);

					rt.scheduler.signalWork();
					BlockingWorkStealingScheduler.unparkInterval = Math.max(BlockingWorkStealingScheduler.unparkInterval / 4, BlockingWorkStealingScheduler.initialUnparkInterval);
